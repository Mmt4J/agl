<div class="space-y-4">
    ...
    <div class="flex flex-wrap gap-2">
        @foreach ([['value' => null, 'label' => 'All'], ['value' => 'unread', 'label' => 'Unread'], ['value' => 'read', 'label' => 'Read'], ['value' => 'replied', 'label' => 'Replied']] as $option)
    ...
    <div
        ...
        @forelse ($this->messages as $message)
            <button type="button" wire:key="message-{{ $message->id }}" wire:click="viewMessage({{ $message->id }})"
                ...
                <span
                    class="mt-1.5 w-2 h-2 rounded-full shrink-0 {{ $message->status === 'unread' ? 'bg-copper-500' : 'bg-transparent' }}"></span>

                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2">
                        <p
                            class="text-sm truncate {{ $message->status === 'unread' ? 'font-semibold text-ink-950 dark:text-linen-50' : 'font-medium text-ink-800 dark:text-linen-100' }}">
                            {{ $message->full_name }}
                        </p>
                        <span
                            class="text-xs text-ink-900/40 dark:text-linen-100/40 shrink-0">{{ $message->created_at->diffForHumans() }}</span>
                    </div>
                    <p class="text-xs text-ink-900/60 dark:text-linen-100/60 truncate">{{ $message->subject }}</p>
                    <p class="text-xs text-ink-900/40 dark:text-linen-100/40 truncate mt-0.5">{{ $message->message }}
                    </p>
                </div>
        ...
        @if ($this->viewingMessage)
            <div class="p-6 space-y-6">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <h2 class="font-display text-lg font-semibold truncate">{{ $this->viewingMessage->full_name }}
                        </h2>
                        <p class="font-mono text-xs text-ink-900/50 dark:text-linen-100/50 truncate">
                            {{ $this->viewingMessage->email }}</p>
                    </div>
                ...
                <a href="mailto:{{ $this->viewingMessage->email }}?subject=Re: {{ $this->viewingMessage->subject }}"
