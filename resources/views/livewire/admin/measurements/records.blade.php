    ...
    <x-forms.panel name="measurement-detail">
        @if ($this->viewingRecord)
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="space-y-6">
                    @foreach ([['title' => 'Upper body', 'rows' => $upper], ['title' => 'Lower body', 'rows' => $lower], ['title' => 'Overall', 'rows' => $overall]] as $group)
                        <div>
                            <p class="font-mono text-[10px] uppercase text-ink-900/40 dark:text-linen-100/40 mb-2">{{ $group['title'] }}</p>
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                @foreach ($group['rows'] as $row)
                                    <div class="rounded-md border border-ink-900/10 dark:border-linen-100/10 px-3 py-2">
                                        ...
                                        <p class="text-sm">{{ $row['value'] !== null ? number_format((float) $row['value'], 1) : '—' }}</p>
                                    </div>
                                @endforeach
                            </div>
                        </div>
