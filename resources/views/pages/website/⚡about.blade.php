...
<div>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 grid lg:grid-cols-2 gap-12">
        ...
        <div class="space-y-5 text-ink-900/70 dark:text-linen-100/70 leading-relaxed">
            <p>Small businesses should not have to choose between a specialist who disappears after payment and a large
                agency that cannot see the work at street level. We built ANESMAVISA to make capable, practical help
                easier to find and easier to trust.</p>
            <p>Every engagement is scoped clearly, routed to the right division and kept on the record. That means
                honest diagnostics for a device, a maintainable system for a business, and a real person accountable for
                the result.</p>
            <div class="border-l-2 border-copper-500 pl-5 pt-1 text-ink-900 dark:text-linen-50 font-display text-xl">
                Good work should leave a clear paper trail.</div>
        </div>
    </section>
    ...
    <section class="bg-ink-900 text-linen-50 border-y border-ink-900/10 dark:border-linen-100/10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5 mb-10">
                <div>
                    <p class="font-mono text-xs uppercase tracking-widest text-copper-300 mb-2">Our operating brief</p>
                    <h2 class="font-display font-semibold text-3xl sm:text-4xl">What we bring to the table</h2>
                </div>
    ...
    @if ($this->testimonials->isNotEmpty())
        <section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
            <p class="font-mono text-xs uppercase tracking-widest text-copper-600 dark:text-copper-300 mb-8">Statements
                on file</p>
            <div class="grid md:grid-cols-3 gap-8">
                @foreach ($this->testimonials as $testimonial)
                    <figure wire:key="about-testimonial-{{ $testimonial->id }}"
                        class="border-t-2 border-copper-500 pt-5">
                        <blockquote class="font-display text-xl leading-relaxed">“{{ $testimonial->quote }}”
                        </blockquote>
                        <figcaption class="mt-5 text-sm"><span
                                class="font-semibold">{{ $testimonial->client_name }}</span><span
                                class="block text-ink-900/50 dark:text-linen-100/50">{{ $testimonial->client_role }}</span>
                        </figcaption>
                    </figure>
                @endforeach
            </div>
