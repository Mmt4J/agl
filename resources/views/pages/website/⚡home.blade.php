...
<div>
    ...
    class="border-b border-ink-900/10 dark:border-linen-100/10 bg-linen-100 dark:bg-ink-900/40">
        ...
        >
            <div class="flex flex-wrap items-center justify-center gap-y-3 sm:gap-y-2">
                ...
                {{-- Desktop separator --}}
                <span
                    class="hidden sm:inline mx-3 text-ink-900/20 dark:text-linen-100/20"
                    aria-hidden="true"
                >/</span>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
            <div>
                <p class="font-mono text-xs uppercase tracking-widest text-copper-600 dark:text-copper-300 mb-2">Schedule A - Services</p>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            @foreach ($this->services as $service)
                <article wire:key="service-{{ $service->id }}" class="group rounded-md border border-ink-900/12 dark:border-linen-100/12 bg-white dark:bg-ink-900/40 p-6 hover:border-copper-500/60 transition-colors">
                    <div class="flex items-start justify-between mb-4">
                        <span class="w-10 h-10 rounded-md bg-ink-900/5 dark:bg-linen-100/5 text-ink-900 dark:text-copper-300 grid place-items-center group-hover:bg-copper-500 group-hover:text-linen-50 transition-colors">
                    ...
                    <p class="text-sm text-ink-900/60 dark:text-linen-100/60 mt-2 leading-relaxed">{{ $service->short_description }}</p>
                </article>
            @endforeach
        </div>
    </section>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 grid lg:grid-cols-2 gap-12 items-center">
            <div class="order-2 lg:order-1">
                ...
                <dl class="grid grid-cols-2 gap-5">
                    ...
                    <dd class="text-xs text-ink-900/60 dark:text-linen-100/60 mt-0.5">RC {{ $this->companySettings['rc_number'] }}, incorporated March 2026.</dd>
                    </div>
                    <div class="border-l-2 border-copper-500 pl-3">
                    <dt class="font-semibold text-sm">SCUML Compliant</dt>
                    <dd class="text-xs text-ink-900/60 dark:text-linen-100/60 mt-0.5">RN {{ $this->companySettings['scuml_number'] }}, anti-money-laundering registered.</dd>
                    </div>
                </dl>
