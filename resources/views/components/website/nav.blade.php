...
--}}
<a href="#main-content" class="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-[100] focus:bg-ink-900 focus:text-linen-50 focus:px-4 focus:py-2 focus:rounded-md">
    Skip to content
</a>
...
>
    <div class="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
        <a href="{{ route('website.home') }}" wire:navigate class="flex items-center gap-2 shrink-0">
            ...
            <span class="leading-none">
                ...
                <span class="block font-mono text-[9px] tracking-widest text-copper-600 dark:text-copper-300">GLOBAL LTD</span>
            </span>
        </a>
        ...
        <div class="ml-auto flex items-center gap-2">
            ...
            >
                ...
                >
                    <svg x-show="theme !== 'dark'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v1.5m0 15V21m8.485-8.485H19M5 12H3.515m13.435 6.364l-1.06-1.06M6.11 6.11l-1.06-1.06m12.02 0l-1.06 1.06M6.11 17.89l-1.06 1.06M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    <svg x-show="theme === 'dark'" x-cloak class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>
                </button>
            </div>

            <a
                href="{{ route('website.quote') }}"
                wire:navigate
            ...
            <button @click="mobileOpen = !mobileOpen" class="lg:hidden w-10 h-10 grid place-items-center rounded-md hover:bg-ink-900/5 dark:hover:bg-linen-100/10" aria-label="Open menu">
    ...
    >
        ...
        @endforeach
        <a href="{{ route('website.quote') }}" wire:navigate class="mt-2 text-center rounded-md bg-copper-500 text-ink-950 font-semibold px-4 py-2.5 text-sm">
            Request a Quote
        </a>
    </nav>
</header>
...
@if ($whatsappNumber)
    <a
        href="https://wa.me/{{ preg_replace('/\D/', '', $whatsappNumber) }}?text={{ urlencode($whatsappMessage) }}"
        target="_blank"
        rel="noopener"
